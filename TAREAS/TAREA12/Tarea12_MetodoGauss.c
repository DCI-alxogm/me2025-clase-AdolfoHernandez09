/* Adolfo Hernández Ramírez 
  ...
  Gauss Simple, y que contenga lo siguiente:
...
int estaBienCondicionado(double **matriz, int n) {
    for (int i = 0; i < n; i++) {
        double suma = 0.0;
        for (int j = 0; j < n; j++) {
            if (i != j) {
...
int main(int argc, char *argv[]) {
    ...
    printf("\n--- INICIANDO ELIMINACION DE GAUSS ---\n");
    
    for (int k = 0; k < n; k++) {
        printf("\n--- ETAPA %d (Pivote en posición %d,%d) ---\n", k + 1, k + 1, k + 1);
        
        // Evaluar necesidad de cambiar filas
        ...
        // Normalizar la fila (hacer el pivote = 1)
    ...
    if (estaBienCondicionado(matriz, n)) {
    ...
    for (int i = n - 1; i >= 0; i--) {
        ...
        printf("\nCalculando x%d:\n", i + 1);
        printf("  Valor inicial: %.3f", solucion[i]);
        
        for (int j = i + 1; j < n; j++) {
            printf(" - (%.3f * %.3f)", matriz[i][j], solucion[j]);
            solucion[i] -= matriz[i][j] * solucion[j];
        }
        
        printf("\n  Dividiendo por %.3f: ", matriz[i][i]);
